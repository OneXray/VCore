...
Issues and specs live in GitHub Issues for `OneVCore/VCore`.

Use `gh`. Every issue or PR command must specify `--repo OneVCore/VCore` because
the SSH remote uses an alias. For `gh api`, use an explicit `repos/OneVCore/VCore/...`
endpoint.

## Operations

- Publish a request/spec: `gh issue create --repo OneVCore/VCore --title "..." --body-file <file>`.
- Read a ticket: `gh issue view <number> --repo OneVCore/VCore --comments`.
- List tickets: `gh issue list --repo OneVCore/VCore --state open --json number,title,body,labels,comments`.
- Comment: `gh issue comment <number> --repo OneVCore/VCore --body-file <file>`.
- Apply/remove labels: `gh issue edit <number> --repo OneVCore/VCore --add-label "..."` / `--remove-label "..."`.
- Close: `gh issue close <number> --repo OneVCore/VCore --comment "..."`.
...
- Claim it with `gh issue edit <number> --repo OneVCore/VCore --add-assignee @me`.
