...
- 可用地址族从非 link-local 地址中选出的源 IP 和非零接口索引；
- 独立于源地址选择保留的全部去重 on-link prefixes，包括 link-local。
...
Provider 订阅网络变化，等待 2 秒消抖后复验适配器、选定源地址、全部 on-link prefixes 和 identity。任一变化就停止会话，不迁移 socket、不重选网卡、不自动回退。
