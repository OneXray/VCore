...
proxies:
  - name: anytls-edge
    ...
    # fingerprint: "<certificate SHA-256, 64 hex digits>"
    udp: true
...
- `alpn` 可省略或为 `[]`，均不发送 ALPN；非空列表按配置顺序发送。每项为 1–255 个 UTF-8 字节，带长度字节的列表总长最多 65,533 字节。
...
- 默认使用 WebPKI 根证书、规范化 SNI 和完整证书校验；
- `skip-cert-verify: true` 且没有 pin 时跳过常规证书链、名称及有效期验证；
- `fingerprint` 匹配叶证书时，以该证书作为信任依据，不再要求公共根、名称或有效期匹配；匹配非叶证书时，以它为信任锚，继续验证叶证书链、名称及有效期；
...
客户端首个 `Settings` 固定声明 `v=2`，并携带当前填充方案的 MD5，不附带产品或客户端版本标识。服务端返回合法的 `ServerSettings v>=2` 时按 v2 工作；未返回时按兼容 v1 的方式继续。
...
- 空闲清理任务在节点首次使用时按需启动；只构建未使用的节点不创建后台任务，建图或监听绑定失败不会遗留空闲 worker。
- 活动物理会话不设固定业务数量上限。
