...
## Windows Phase 3A 产品包验收（2026-08-23）
...
- 正常 `lib/main.dart` UI 也已用无凭据 Full Config 完成连接，IPv4/IPv6 local ICMP、DNS namespace、TCP DNS、Aliyun NTP 与 Baidu HTTP 均通过。强制关闭并重开 foreground 后 UI 恢复 `VCore Config` connected session；Stop 删除 session record、routes 与 dirty provider。随后十轮 UI rapid reconnect 每轮 provider PID 均不同，且 TCP DNS/UDP NTP 每轮通过。强制结束 active provider 后，route 与 foreground session record 在 370 ms 内消失，UI 保持 disconnected 且未自动重连。App Proxy mode 现限定为 iOS，Windows 与其他平台只保留 TUN；重建包已由用户人工确认 Windows Core 页无 Proxy selector，托盘也无 mode/Proxy 项。
- production Dart worker-isolate FFI path 已向 ARM64 `vcore.dll` 提交两个项目的 `measureDelay` batch；使用临时、无凭据的 loopback SOCKS5 fixture，两项按序成功返回 263/270 ms。probe、listener、data directory 与 fixture 随后全部删除。
- 正常 Desktop settings UI 已完成 StartupTask enable/query/disable：启用后 registry state 为 2；真实注销并重新登录后，Explorer 于 20:05:16 启动，package-owned `OneVCore.exe` 于 20:05:29 自动启动，且未意外创建 provider、VPN route 或 session record；关闭开关后 state 为 0，当前 App 保持运行。
...
- VCore Windows lib tests 为 427 passed / 1 ignored；lib Clippy `-D warnings`、ARM64/x64 release DLL 与 provider-host build、Flutter ARM64 release build、MSIX make/sign/install、PowerShell parser、focused Dart/Python contract tests通过。最终 ARM64/x64 `vcore.dll` SHA-256 为 `03aa722f6317db9ee918f9fd4fa3bc7529c49c0daa087a0765f6e0cccf1400f4` / `0e9b633d545e87bc09f580857b78dfd0d81eef6c2eaeaedb37e27d5be1d80946`；对应 host 为 `6c42ef5ebb71ce4ff904f1eabcba745fac3c5507565987b398a0084c8307fc97` / `1ced15314f593e803d440ae94f618b4fa2b4f8199bfacb83ba0cd2af8b119d39`；包含 iOS-only Proxy gate 的 Phase 3A ARM64 Dev MSIX 为 `0412b6d390ae2111af29f483d0988b65f64ce0e5050fed704f157e1082c9a72c`。全 target Clippy 仍命中既有 test-only unused imports/dead code；Apple-only C header shell check 在 Windows 无 `xcrun`，两者不记为通过。
- explicit Stop 后的开发 probe uninstall 已通过。一次主动删除仍在运行的 probe package 会先停止 VPN，但本机 package 状态随后停留在 `DeploymentInProgress, Servicing`；重启已清除该状态，package script 也已拒绝在 VPN route active 时 install/replace。Phase 3 不承诺 active upgrade/uninstall。
