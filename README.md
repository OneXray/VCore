...
- 产物身份：`version` 返回 `engine: rust`、Invoke API 4、内部 schema revision 11 和稳定 `buildIdentity`；revision 11 不写入 YAML。Apple/Android 构建及 App 同步在复制前核对该身份，源码 revision 与产物 hash 由发布记录单独保存。
